...
import { computed, getCurrentInstance, nextTick, ref, watch, type ExtractPropTypes } from 'vue';
import { _propsOptions, type VPopupProps } from './popup';
...
const handleTask = (task: Function) => {
  if (isVisible.value) {
    task()
  } else {
    tasks.push(task)
...
const dialogAttrs = computed(() => {
  const attrs = { ...vm?.attrs } as Mutable<InstanceType<typeof ElDialog>['$props']>
...
<ElDialog v-model="isVisible" v-bind="dialogAttrs" >
  <template #header="scoped">
    <slot name="header" v-bind="scoped">
      <span :id="scoped.titleId" :class="scoped.titleClass">{{ dialogAttrs.title }}</span>
      <span
        v-if="fullscreenable"
